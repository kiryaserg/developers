...
class TeamTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testReport()
    {
        ...
        $this->assertStringStartsWith(
            'Владислава (junior) Аркадий (junior) Игорь (junior) Олеся (developer) Василий (developer) Оля (developer) Олег (senior) Оксана (senior)',
            $result
        );

        foreach (array_merge($team->juniors()) as $dev) {
            $team->add_task('hello');
        };
        ob_start();
        $team->report();
        $result = ob_get_clean();
        $this->assertStringStartsWith(
            "Олеся (developer) Василий (developer) Оля (developer) Олег (senior) Оксана (senior) Игорь (junior) 0 hello
Аркадий (junior) 0 hello
Владислава (junior) 0 hello
",
            $result
        );
    }
